...
class UserTest(CssefTest):
    def test_create_minimal_user(self):
        user_object = create_org_and_user(self.db_connection)
    ...
    def test_delete_user(self):
        user_object = create_org_and_user(self.db_connection)
        user_object.delete()
        user_object1 = User.from_database(self.db_connection, user_object.get_id())
...
class OrganizationTest(CssefTest):
    def test_create_minimal_org(self):
        ...
        org_object = Organization.from_dict(self.db_connection, org_dict)

def create_org_and_user(db_connection):
    org_dict = { "name": "testorg", "max_members": 10 }
    org_object = Organization.from_dict(db_connection, org_dict)
    user_dict = {
        'username': "bob",
        "name": "Bob",
        "password": "bobpass",
    ...
    return User.from_dict(db_connection, user_dict)
